...
describe('State', function () {
  ...
  it('should get block parent receipts [ws]', async function () {
    ...
    const receipts = await provider.chain.getParentReceipts(
      blocksWithMessages[0],
    );
    assert.strictEqual(
      typeof receipts[0].GasUsed,
      'number',
  ...
  it('should get block parent messages [http]', async function () {
    ...
    assert.strictEqual(
      typeof messages[0].Message.Nonce,
      'number',
      'invalid message',
    );
  });
  ...
  it('should get block parent messages [ws]', async function () {
    const provider = new LotusClient(wsConnector);
    const messages = await provider.chain.getParentMessages(
      blocksWithMessages[0],
    );
    assert.strictEqual(
      typeof messages[0].Message.Nonce,
  ...
  it('should get state [http]', async function () {
    ...
    assert.strictEqual(
      ...
      JSON.stringify(['Balance', 'Code', 'State']),
      'invalid state',
    );
  });
  ...
  it('should get miner sectors info [http]', async function () {
    ...
    const valid = sectors.reduce(
      (acc, sector) =>
        acc === false ? acc : typeof sector.SectorNumber === 'number',
      true,
    );
  ...
  it('should get miner proving deadline', async function () {
    ...
    assert.strictEqual(
      typeof provingDeadline.Index === 'number',
      true,
      'invalid miner proving deadline',
    );
  });
  ...
  it('should get miner power', async function () {
    ...
    assert.strictEqual(
      typeof power.MinerPower.RawBytePower === 'string',
      true,
      'invalid miner power',
    );
  });
  ...
  it('should get miner deadlines', async function () {
    ...
    const valid = minerDeadlines.reduce(
      (acc, deadline) =>
        acc === false ? false : Array.isArray(deadline.PostSubmissions),
      true,
    );
  ...
  it('should get miner partitions', async function () {
    ...
    const valid = minerPartitions.reduce(
      (acc, partition) =>
        acc === false ? false : Array.isArray(partition.AllSectors),
      true,
    );
  ...
  it('should get the faulty sectors of a miner', async function () {
    ...
    assert.strictEqual(
      minerFaults === null || Array.isArray(minerFaults),
      true,
      'invalid miner faulty sectors',
    );
  });
  ...
  it('should get the recovering sectors of a miner', async function () {
    ...
    assert.strictEqual(
      recoveries === null || Array.isArray(recoveries),
      true,
      'invalid miner recovering sectors',
    );
  });
  ...
  it("should get the precommit deposit for the specified miner's sector", async function () {
    ...
    assert.strictEqual(
      ...
      true,
      "invalid precommit deposit for the specified miner's sector",
    );
  });
  ...
  it("should get the initial pledge collateral for the specified miner's sector", async function () {
    ...
    assert.strictEqual(
      ...
      true,
      "invalid pledge collateral for the specified miner's sector",
    );
  });
  ...
  it("should get the miner's balance that can be withdrawn or spent", async function () {
    ...
    assert.strictEqual(
      ...
      true,
      "invalid miner's balance that can be withdrawn or spent",
    );
  });
  ...
  it("should return info for the specified miner's sector", async function () {
    ...
    assert.strictEqual(
      info.SectorNumber === 0,
      true,
      "invalid info for the specified miner's sector",
    );
  });
  ...
  it('should get epoch at which given sector will expire', async function () {
    ...
    assert.strictEqual(
      typeof expiration.OnTime === 'number',
      true,
      'invalid epoch at which given sector will expire',
    );
  });
  ...
  it('should get deadline/partition with the specified sector', async function () {
    ...
    assert.strictEqual(
      typeof partition.Partition === 'number',
      true,
      'invalid deadline/partition with the specified sector',
    );
  });
  ...
  it('should search for message and return its receipt and the tipset where it was executed', async function () {
    ...
    assert.strictEqual(
      typeof receipt.Height === 'number',
      true,
      'invalid height for searched messages',
    );
  });
  ...
  it('should wait for message', async function () {
    ...
    assert.strictEqual(
      typeof lookup.Height === 'number',
      true,
      'invalid lookup info for waited messages',
    );
  });
  ...
  it('should wait message limited', async function () {
    ...
    assert.strictEqual(
      typeof lookup.Height === 'number',
      true,
      'invalid lookup info for limited waited messages',
    );
  });
  ...
  it('should get market balance', async function () {
    ...
    assert.strictEqual(
      typeof marketBalance.Escrow === 'string',
      true,
      'invalid market balance',
    );
  });
  ...
  */
  ...
  it('should get information about the storage deal', async function () {
    ...
    assert.strictEqual(
      !!marketDeal.Proposal.PieceCID,
      true,
      'invalid information about the storage deal',
    );
  });
  ...
  it('should return min and max collateral a storage provider can issue', async function () {
    const con = new LotusClient(httpConnector);
    const collateralBounds = await con.state.dealProviderCollateralBounds(
      Math.pow(1024, 2),
      true,
    );
    const valid =
      typeof collateralBounds.Min === 'string' &&
      typeof collateralBounds.Max === 'string';
    assert.strictEqual(
      valid,
      true,
      'invalid collateral a storage provider can issue',
    );
  });
  ...
  it('the circulating supply of Filecoin at the given tipset', async function () {
    ...
    const valid = Object.keys(supply).reduce(
      (acc, key) => (acc === false ? acc : typeof key === 'string'),
      true,
    );
  ...
  it('the vm circulating supply of Filecoin at the given tipset', async function () {
    ...
    assert.strictEqual(
      valid,
      true,
      'invalid vm circulating supply of Filecoin',
    );
  });
