...
async function fundTestAddresses(
  source: string,
  destinations: string[],
  walletLotusHttp: LotusWalletProvider,
  con: LotusClient,
) {
...
describe('Multisig Wallets Mnemonic implementation', function () {
  it('should provide accounts with initial funds', async function () {
    ...
    await fundTestAddresses(
      await walletLotusHttp.getDefaultAddress(),
      addresses,
      walletLotusHttp,
      con,
    );
  ...
  it('should create multisig wallet and transfer funds [http]', async function () {
    ...
    const multisigCid = await mnemonicWalletProvider.msigCreate(
      2,
      addresses,
      0,
      0,
    ...
    const balance = await mnemonicWalletProvider.msigGetAvailableBalance(
      multisigAddress,
      [],
    );
    assert.strictEqual(balance, '1000', 'wrong balance');

    const initTransferCid = await mnemonicWalletProvider.msigProposeTransfer(
      multisigAddress,
      mnemonicAddress,
      '1',
      addresses[0],
    ...
    const approveTransferCid = await mnemonicWalletProvider.msigApproveTransferTxHash(
      multisigAddress,
      txnID,
      addresses[0],
      mnemonicAddress,
    ...
    const balanceAfterTransfer = await mnemonicWalletProvider.msigGetAvailableBalance(
      multisigAddress,
      [],
    );
    assert.strictEqual(
  ...
  it('should create multisig wallet and add signer [http]', async function () {
    ...
    const initAddProposeCid = await mnemonicWalletProvider.msigProposeAddSigner(
      multisigAddress,
      addresses[0],
      addresses[2],
      true,
    ...
    const approveAddCid = await mnemonicWalletProvider.msigApproveAddSigner(
      multisigAddress,
      addresses[1],
      txnID,
      addresses[0],
  ...
  it('should create multisig wallet and swap signer [http]', async function () {
    ...
    const initSwapProposeCid = await mnemonicWalletProvider.msigProposeSwapSigner(
      multisigAddress,
      addresses[0],
      addresses[1],
      addresses[2],
    );
    const receiptSwapProposeCid = await con.state.waitMsg(
      initSwapProposeCid,
      0,
    );
    ...
    const approveSwapCid = await mnemonicWalletProvider.msigApproveSwapSigner(
      multisigAddress,
      addresses[1],
      txnID,
      addresses[0],
  ...
  it('should create multisig wallet and cancel transfer funds [http]', async function () {
    ...
    const cancelTransferCid = await mnemonicWalletProvider.msigCancelTransfer(
      multisigAddress,
      addresses[0],
      txnID,
      mnemonicAddress,
    ...
    assert.strictEqual(
      receiptTransferCancel.Receipt.ExitCode,
      0,
      'error canceling transfer',
    );
  });
  ...
  it('should create multisig wallet cancel add signer [http]', async function () {
    ...
    const cancelAddCid = await mnemonicWalletProvider.msigCancelAddSigner(
      multisigAddress,
      addresses[0],
      txnID,
      addresses[2],
    ...
    assert.strictEqual(
      receiptAddCancel.Receipt.ExitCode,
      0,
      'error canceling add signer',
    );
  });
  ...
  it('should create multisig wallet and cancel swap signer [http]', async function () {
    ...
    const cancelSwapCid = await mnemonicWalletProvider.msigCancelSwapSigner(
      multisigAddress,
      addresses[0],
      txnID,
      addresses[1],
    ...
    assert.strictEqual(
      receiptSwapCancel.Receipt.ExitCode,
      0,
      'error canceling swap proposal',
    );
  });
  ...
  it('should create multisig wallet and remove signer [http]', async function () {
    ...
    const initRemoveProposeCid = await mnemonicWalletProvider.msigProposeRemoveSigner(
      multisigAddress,
      addresses[0],
      addresses[2],
      true,
    ...
    const approveRemoveCid = await mnemonicWalletProvider.msigApproveRemoveSigner(
      multisigAddress,
      addresses[1],
      txnID,
      addresses[0],
    ...
    assert.strictEqual(
      receiptRemoveApprove.ReturnDec.Applied,
      true,
      'error approving add proposal',
    );
  });
  ...
  it('should create multisig wallet and cancel remove signer [http]', async function () {
    ...
    const cancelRemoveCid = await mnemonicWalletProvider.msigCancelRemoveSigner(
      multisigAddress,
      addresses[0],
      txnID,
      addresses[2],
    ...
    assert.strictEqual(
      receiptRemoveCancel.Receipt.ExitCode,
      0,
      'error canceling remove proposal',
    );
  });
});
