...
describe('Chain methods', function () {
  it('statistics about the graph', async () => {
    ...
    const diffStat = await provider.chain.statObj(
      tipset2.Cids[0],
      tipset1.Cids[0],
    );
    const isStatValid = (stat: ObjStat) =>
      typeof stat.Size === 'number' && typeof stat.Links === 'number';
    assert.strictEqual(
      isStatValid(stat) && isStatValid(diffStat),
      true,
      'invalid obj statistics',
    );
  });
  ...
  it('get tipSet weight', async () => {
    ...
    assert.strictEqual(
      typeof tipSetWeight === 'string',
      true,
      'invalid tipSet weight',
    );
  });
  ...
  it('get path', async () => {
    ...
    const tipSet2 = await provider.chain.getTipSetByHeight(10);
    const path = await provider.chain.getPath(
      [tipSet.Cids[0]],
      [tipSet2.Cids[0]],
    );
    const typeValid = path.reduce(
      (acc, pathItem) =>
        acc === false ? acc : typeof pathItem.Type === 'string',
      true,
    );
  ...
  it('should get messages in block [ws]', async function () {
    ...
    assert.strictEqual(
      ...
      JSON.stringify(['BlsMessages', 'SecpkMessages', 'Cids']),
      'wrong block messages',
    );
  ...
  it('should get head [http]', async function () {
    ...
    assert.strictEqual(
      JSON.stringify(Object.keys(head)),
      JSON.stringify(['Cids', 'Blocks', 'Height']),
      'wrong chain head',
    );
  });
  ...
  it('should check obj exists in the chain [http]', async function () {
    ...
    assert.strictEqual(
      isInChain,
      true,
      "CID doesn't exists in the chain blockstore",
    );
  });
  ...
  it('should check tipset after height [http]', async function () {
    ...
    assert.strictEqual(
      typeof tipSet.Height === 'number',
      true,
      'invalid tipset after height',
    );
  });
  ...
  it('should get estimate gas limit [http]', async function () {
    ...
    let message;
    do {
      message = await con.chain.getMessage(messages[i]);
      i++;
